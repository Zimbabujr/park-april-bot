...
from bot.keyboards.main import (
    main_menu_keyboard, create_ride_keyboard, seats_keyboard,
    price_type_keyboard, confirm_ride_keyboard, search_filters_keyboard,
    ride_actions_keyboard, booking_request_keyboard, back_keyboard,
    my_rides_keyboard, ride_detail_keyboard, rating_keyboard
)
...
async def create_ride_start(message: Message, state: FSMContext, user: User = None):
    ...
    if not user or not user.is_driver:
        await message.answer(
            "❌ Только пользователи с автомобилем могут создавать поездки.
"
            "Если у вас есть авто, обновите данные в профиле."
        )
        return
...
async def publish_ride(callback: CallbackQuery, state: FSMContext, bot: Bot, user: User = None):
    """Публикация поездки"""
    data = await state.get_data()
...
"
            f"📝 Для бронирования напишите боту: @{bot._me.username}"
        )
